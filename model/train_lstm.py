...
EPOCHS = 50
BATCH_SIZE = 16
PATIENCE = 5 # untuk EarlyStopping
...
# Fungsi untuk melatih, menyimpan, dan memplot model LSTM 
def train_and_plot_lstm_model(drug_name, df_path):
    print(f"\n--- Memulai pelatihan LSTM untuk {drug_name} ---")
    ...
    # Bangun model LSTM
    model = Sequential()
    model.add(LSTM(64, activation='relu', input_shape=(WINDOW_SIZE, 1))) # Menggunakan activation relu
    model.add(Dense(1))
    model.compile(optimizer='adam', loss='mse')

    print(f"Summary model LSTM untuk {drug_name}:")
    model.summary()
    ...
    model.save(os.path.join(drug_model_dir, 'lstm_model.h5'))
    joblib.dump(scaler, os.path.join(drug_model_dir, 'scaler_lstm.gz'))

    print(f"✅ LSTM model dan scaler untuk {drug_name} disimpan di: {drug_model_dir}")

    # Plot training loss
    plt.figure(figsize=(10, 6))
    plt.plot(history.history['loss'], label='Training Loss')
    plt.plot(history.history['val_loss'], label='Validation Loss')
    plt.title(f'LSTM Model Loss for {drug_name}')
    plt.xlabel('Epoch')
    plt.ylabel('Loss (MSE)')
    plt.legend()
    plt.grid(True)
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(DATASET_DIR):
        if filename.endswith('.csv'):
            ...
            train_and_plot_lstm_model(drug_name, file_path)